/**
 * Sea-route arc generation.
 *
 * Straight lines between an Asian supplier and a US port cut across continents.
 * This builds a realistic-looking ocean route that bends through the right
 * canals/straits (Panama, Suez, Gibraltar, Malacca, Cape of Good Hope), then
 * densifies each leg with great-circle interpolation so the line curves like a
 * real voyage.
 *
 * This is a pragmatic gazetteer-driven router, NOT a full bathymetric pathfinder
 * (e.g. searoute / Pandana). It's tuned for the trial's dominant lanes
 * (East Asia / Med-Europe / South Asia → US East & West coasts). When the live
 * feed lands, real AIS tracks replace these arcs entirely — see
 * pericles-atlas-mocker.
 */
...
/**
 * Choose canal/strait waypoints between origin and destination. Heuristic, lane-
 * tuned; returns the intermediate points only (origin/dest appended by caller).
 */
function chooseWaypoints(origin: LatLng, dest: LatLng): LatLng[] {
  ...
  if (isMedOrEurope(origin) && isUsEastOrGulf(dest)) {
    return isMediterranean(origin)
      ? [WP.GIBRALTAR, WP.ATLANTIC_MID]
      : [WP.ATLANTIC_MID];
  }
 ...
 *
 * @param pointsPerLeg approximate vertices per leg (scaled by leg length).
 */
...
export function cumulativeDistances(polyline: LatLng[]): { cumulativeKm: number[]; totalKm: number } {
  const cumulativeKm: number[] = [0];
  for (let i = 1; i < polyline.length; i++) {
    cumulativeKm.push(cumulativeKm[i - 1] + haversineKm(polyline[i - 1], polyline[i]));
  }
  return { cumulativeKm, totalKm: cumulativeKm[cumulativeKm.length - 1] ?? 0 };
}

/**
 * Position + bearing at a given covered distance along the polyline.
 */
export function pointAtDistance(
  ...
  const target = Math.max(0, Math.min(coveredKm, total));

  // Binary-ish linear scan (polylines are small).
  let i = 1;
  while (i < cumulativeKm.length && cumulativeKm[i] < target) i++;
  const a = polyline[i - 1];
  const b = polyline[Math.min(i, polyline.length - 1)];
  const segStart = cumulativeKm[i - 1];
  const segLen = (cumulativeKm[i] ?? segStart) - segStart || 1;
  const t = (target - segStart) / segLen;
  return { position: slerp(a, b, Math.max(0, Math.min(1, t))), bearing: bearingDeg(a, b) };
}
