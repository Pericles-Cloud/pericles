...
export function loadTrackingConfig(env: NodeJS.ProcessEnv = process.env): TrackingConfig {
  ...
  // 2000 → a trans-Pacific crossing takes ~10 min of wall time: dots advance
  // perceptibly across a session without the frantic ~2 min ocean crossing
  // that 20000 produced. GH #9 removed the fast-moving arrow glyphs and asked
  // for the ship to sit where it actually is, so the mock's pace should read
  // like a ship, not a drone. Override per env.
