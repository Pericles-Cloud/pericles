...
export function normalizeRecord(r: RawRecord): BolRow | null {
  ...
  return {
    ...
    vessel_name: str(r.vessel_name) ?? str(r.vesselName) ?? str(r.vessel) ?? str(r.ship_name),
    vessel_code: str(r.vessel_imo) ?? str(r.vesselImo) ?? str(r.imo),
    voyage: str(r.voyage) ?? str(r.voyage_number) ?? str(r.voyageNumber),
    value_usd: num(r.value_usd) ?? num(r.valueUsd) ?? num(r.customs_value),
